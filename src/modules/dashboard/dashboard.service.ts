...
export const getWeeklyTrends = async (userId: string, userRole: string) => {
  try {
    ...
    const records = await prisma.financialRecord.findMany({
      where: userRole === 'ADMIN'
        ? { isDeleted: false, date: { gte: eightWeeksAgo } }
        : { isDeleted: false, createdById: userId, date: { gte: eightWeeksAgo } },
      select: { amount: true, type: true, date: true },
    });
...
export const getBalanceOverTime = async (userId: string, userRole: string) => {
  try {
    ...
    console.error('getBalanceOverTime error:', error);
    throw error;
  }
};
  ...
  try {
    const records = await prisma.financialRecord.findMany({
      where: userRole === 'ADMIN'
        ...
        : { isDeleted: false, createdById: userId, type },
